          ...
          <button 
            onClick={async () => {
              try {
                ...
                const resp = await apiFetch<{ dataset_id: string }>("/datasets/seed-demo", { method: "POST" });
                if (resp.dataset_id) {
                  const { toast } = await import("sonner");
                  toast.success("Demo dataset initialising — it'll appear shortly.");
                }
              } catch {
                const { toast } = await import("sonner");
                toast.error("Couldn't start the demo dataset. Please try again.");
              }
            }}
