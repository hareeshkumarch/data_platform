...
const Insights = () => {
  ...
  const activeDatasetName = datasets.find(d => d.id === activeTargetId)?.name || mainDataset?.name || "the dataset";
  ...
  <section className="space-y-4">
    {findings.map((f, i) => (
      <InsightCard key={i} tag={f.tag} title={f.title} explanation={f.explanation}
        confidence={f.confidence} metrics={f.metrics} defaultOpen={true} delay={i * 60}>
      </InsightCard>
    ))}
