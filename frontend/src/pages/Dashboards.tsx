        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Widget title="Primary Dataset distribution" wide delay={240}>
            {mainDataset && mainDataset.rows?.length > 1 ? (
              (() => {
                const numericCol = (mainDataset as any).columns?.find((c: any) => c.inferred_type === "numeric")?.name 
                                  || Object.keys(mainDataset.rows?.[0] || {})[1] 
                                  || "value";
                const dateCol = (mainDataset as any).columns?.find((c: any) => c.inferred_type === "datetime" || c.inferred_type === "string")?.name 
                                  || Object.keys(mainDataset.rows?.[0] || {})[0] 
                                  || "index";
                
                const parsedData = mainDataset.rows.map(row => ({
                  ...row,
                ...
                return (
                  <DynamicChart spec={{ 
                    ...
                    data: parsedData,
                    xKey: dateCol,
                    series: [{ key: numericCol, label: numericCol }],
                    height: 280 
                  }} />
                );
              })()
