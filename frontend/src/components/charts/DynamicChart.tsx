...
export const DynamicChart = ({ spec }: { spec: ChartSpec }) => {
  const { chart, data, xKey, series: rawSeries, height = 240, title } = spec;
  ...
  const Title = (title && spec.showTitle !== false) ? (
  ...
  if (chart === "pie" || chart === "donut") {
    ...
    const pieKey = series?.[0]?.key || Object.keys(firstRow).find((k) => k !== xKey) || "value";
    return (
      <div>
        {Title}
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={height}>
          <ScatterChart margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
            ...
            <YAxis {...axisDefaults} width={65} dataKey={series?.[0]?.key} name={series?.[0]?.label} />
            <Tooltip contentStyle={tooltipStyle} cursor={{ strokeDasharray: "3 3" }} />
            {Array.isArray(series) && series.map((s, i) => (
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={height}>
          <ScatterChart margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} />
            <XAxis dataKey={xKey} {...axisDefaults} name={xKey} />
            <YAxis {...axisDefaults} width={65} dataKey={series?.[0]?.key} />
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={height}>
          <RadarChart data={data} cx="50%" cy="50%" outerRadius={height * 0.35}>
            ...
            <Tooltip contentStyle={tooltipStyle} />
            {Array.isArray(series) && series.length > 1 && <Legend wrapperStyle={{ fontSize: 11 }} iconType="circle" iconSize={8} />}
          </RadarChart>
        </ResponsiveContainer>
      </div>
    );
  ...
  if (chart === "histogram") {
    const firstRow = data[0] || {};
    const histKey = series?.[0]?.key || Object.keys(firstRow).find((k) => k !== xKey) || "value";
    return (
      <div>
        {Title}
        <ResponsiveContainer width="100%" height={height}>
  ...
  if (chart === "heatmap") {
    const firstRow = data[0] || {};
    const numCols = Object.keys(firstRow).filter((k) => k !== xKey);
    return (
      <div>
        {Title}
  ...
  if (chart === "treemap") {
    const firstRow = data[0] || {};
    const valueKey = series?.[0]?.key || Object.keys(firstRow).find((k) => k !== xKey && k !== "name") || "value";
    const treemapData = data.filter(d => d).map((d: any) => ({ name: d[xKey] || d.name || "", size: d[valueKey] || d.value || 0 }));
    return (
      <div>
        {Title}
        <ResponsiveContainer width="100%" height={height}>
  ...
  if (chart === "funnel") {
    const firstRow = data[0] || {};
    const funnelKey = series?.[0]?.key || Object.keys(firstRow).find((k) => k !== xKey && k !== "name") || "value";
    const funnelData = data.filter(d => d).map((d: any, i: number) => ({
  ...
  if (chart === "waterfall") {
    const firstRow = data[0] || {};
    const wfKey = series?.[0]?.key || Object.keys(firstRow).find((k) => k !== xKey) || "value";
    let running = 0;
        ...
        <ResponsiveContainer width="100%" height={height}>
          <ComposedChart data={wfData} margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
            ...
            <Bar dataKey="_val" stackId="wf" fill={palette[0]} radius={[3, 3, 0, 0]} animationDuration={800}>
              {Array.isArray(wfData) && wfData.map((d: any, i: number) => (
