...
export const TopBar = ({
  ...
  const { fetchDatasets: fetchSharedDatasets } = useDatasetStore();
  ...
  useEffect(() => {
    const fetchDatasets = async () => {
      try {
        ...
        setDatasets(list);
        // Validate persisted dataset ID (#55)
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="hidden lg:flex items-center gap-2 h-full px-3 border-r border-border text-xs text-foreground hover:bg-muted/50 transition-colors shrink-0 outline-none">
                ...
                <span className="font-mono truncate max-w-[140px]">{dataset}</span>
              ...
              {datasets.length === 0 ? (
                ...
                datasets.map((d) => (
                  <DropdownMenuItem
                    ...
                    onClick={() => setDataset(d.id || d.dataset_id)}
