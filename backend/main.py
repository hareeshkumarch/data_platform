...
async def lifespan(app: FastAPI):
    logger.info("Starting Lumen", version=settings.APP_VERSION, provider=settings.DEFAULT_LLM_PROVIDER)
    start_metrics_server()
    try:
        SQLWarehouse.get().bootstrap()
    except Exception as exc:  # pragma: no cover - degrade gracefully when PG unavailable
