    ...
    def create(self, name: str) -> _TaskRecord:
        ...
        with self._lock:
            ...
            if len(self._records) > self._max_records:
                oldest_id = min(self._records, key=lambda k: self._records[k].started_at)
...
class _AsyncTask:
    ...
    def apply_async(self, kwargs: Optional[Dict[str, Any]] = None, queue: Optional[str] = None) -> _AsyncTaskHandle:
        kwargs = kwargs or {}
        ...
        async def _runner() -> None:
            ...
            try:
                ...
                rec.finished_at = time.time()
                logger.error("task failed", task=self.name, id=rec.id, error=rec.error, tb=traceback.format_exc())
...
async def _run_ingest(rec: _TaskRecord, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = await agent.execute(payload, payload.get("correlation_id", str(uuid.uuid4())))
    if result.success:
        ...
        try:
            ...
            logger.warning("Vector index failed — proceeding without RAG", error=str(exc))
...
async def _run_process(rec: _TaskRecord, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    feat_result = await FeatureAgent(cache).execute({"dataset_id": payload["dataset_id"]}, corr)
    return {
        "eda": {"success": eda_result.success, "data": eda_result.data, "error": eda_result.error},
        "feature": {"success": feat_result.success, "data": feat_result.data, "error": feat_result.error},
    }
...
async def _run_insights(rec: _TaskRecord, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = await InsightAgent(cache, llm).execute(payload, payload.get("correlation_id", str(uuid.uuid4())))
    return {"success": result.success, "data": result.data, "error": result.error}
...
async def _run_query(rec: _TaskRecord, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = await QueryAgent(cache, llm, vector).execute(payload, payload.get("correlation_id", str(uuid.uuid4())))
    return {"success": result.success, "data": result.data, "error": result.error}
...
async def _run_dashboard(rec: _TaskRecord, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = await VisualizationAgent(cache, llm).execute(payload, payload.get("correlation_id", str(uuid.uuid4())))
    return {"success": result.success, "data": result.data, "error": result.error}
...
async def _run_report(rec: _TaskRecord, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = await ReportAgent(cache, llm).execute(payload, payload.get("correlation_id", str(uuid.uuid4())))
    return {"success": result.success, "data": result.data, "error": result.error}
...
async def _run_pipeline(rec: _TaskRecord, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = await orchestrator.execute(payload, payload.get("correlation_id", str(uuid.uuid4())))
    return {"success": result.success, "data": result.data, "error": result.error}
