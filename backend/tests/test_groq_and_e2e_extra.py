"""Additional end-to-end tests for Groq provider support, dashboard/query/WS
pipeline flows, and graceful-error behaviour when GROQ_API_KEY is absent.

Complements test_lumen_e2e.py.
"""
...
# ---------------- Models/providers: Groq exposure ----------------
...
def test_settings_providers_exposes_groq_key_flags():
    ...
    # The emergent key is required by problem statement
    assert body.get("key_configured") is True
    assert "key_masked" in body
    # Groq-specific fields must be exposed even when empty
...
def test_metrics_llm_key_source_emergent():
    r = requests.get(f"{API}/metrics/llm", timeout=30)
    assert r.status_code == 200
    body = r.json()
    # key_source should be one of a known set; currently 'emergent' because EMERGENT_LLM_KEY is set
    assert body.get("key_source") in ("emergent", "openai", "anthropic", "gemini", "groq", "none"), body
    assert body["key_source"] == "emergent"


# ---------------- Settings POST accepts groq provider ----------------

def test_settings_accepts_groq_provider_whitelist():
    # POST groq — must be accepted even though key is missing (validation is on whitelist)
    r = requests.post(
        f"{API}/settings",
        json={"provider": "groq", "model": "llama-3.3-70b-versatile"},
        timeout=30,
    )
    assert r.status_code in (200, 201), f"groq whitelist rejected: {r.status_code} {r.text}"
    ...
    # revert to openai so downstream tests keep working
    r2 = requests.post(
        f"{API}/settings",
...
# ---------------- Groq without key returns graceful 500 mentioning GROQ_API_KEY ----------------

def test_chat_groq_without_key_returns_clear_error():
    r = requests.post(
        f"{API}/chat",
        json={"prompt": "hello", "provider": "groq", "model": "llama-3.3-70b-versatile"},
        timeout=60,
    )
    # Expected to error since GROQ_API_KEY is empty — we assert the message is informative.
    assert r.status_code in (400, 500, 502, 503), f"unexpected status: {r.status_code} {r.text}"
    text = r.text.lower()
    assert "groq_api_key" in text or "groq api key" in text or "groq" in text, f"error text not helpful: {r.text}"


# ---------------- Dataset + downstream pipeline ----------------
...
def dataset_id() -> str:
    ...
    while time.time() < deadline:
        ...
        if tb.get("success") is True or tb.get("status") in ("success", "SUCCESS", "completed", "done"):
            break
...
def test_process_data_and_eda(dataset_id):
    ...
    # either returns task or direct result
    assert r.status_code in (200, 201, 202), r.text
    body = r.json()
...
def test_generate_and_fetch_insights(dataset_id):
    ...
    ibody = ir.json()
    insights = ibody if isinstance(ibody, list) else ibody.get("insights") or ibody.get("items") or []
...
def test_chart_generate_shape(dataset_id):
    ...
    body = r.json()
    # must have some type and data
    chart_type = body.get("chart_type") or body.get("type") or (body.get("config") or {}).get("chart_type")
    data = body.get("data") or (body.get("config") or {}).get("data") or body.get("rows")
...
def test_query_natural_language(dataset_id):
    ...
    # Either rows or plan/sql should be present
    has_payload = any(k in body for k in ("rows", "data", "sql", "plan", "result", "answer"))
...
def test_ws_pipeline_flow():
    ...
    ws_url = BASE.replace("https://", "wss://").replace("http://", "ws://") + "/api/v1/ws/pipeline"
    stages = []
    complete = None
    with connect(ws_url, open_timeout=15) as ws:
        ...
        while time.time() < deadline:
            ...
            if frame.get("type") == "complete" or frame.get("done") or frame.get("final"):
                complete = frame
                break
            if t:
                stages.append(t)
