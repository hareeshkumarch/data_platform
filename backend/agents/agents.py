...
from backend.models.schemas import AgentType, LLMMode, LLMProvider, LLMRequest
from backend.prompts.templates import (
    INSIGHT_SYSTEM, QUERY_SYSTEM, VIZ_SYSTEM, REPORT_SYSTEM,
    build_insight_prompt, build_query_prompt, build_viz_prompt, build_report_prompt,
)
...
from backend.utils.data_utils import (
    compute_correlation, compute_quality_score, compute_summary_stats,
    detect_anomalies_iqr, detect_anomalies_zscore, infer_schema, parse_csv,
    parse_json, sanitize_rows, smart_sample, time_series_decompose,
    compute_moving_averages, naive_forecast, contribution_analysis, _convert,
)
...
class IngestionAgent(BaseAgent):
    ...
    async def run(self, payload: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
        ...
        return {"dataset_id": dataset_id, "schema": schema, "validation": validation, "sample_row_count": len(sample_rows)}

    async def _load(self, payload: Dict, source_type: str) -> pd.DataFrame:
        content = payload.get("content")
        filepath = payload.get("filepath")
        if source_type == "csv":
            if content: return await asyncio.to_thread(parse_csv, content)
            if filepath: return await asyncio.to_thread(pd.read_csv, filepath)
        elif source_type == "json":
            if content: return await asyncio.to_thread(parse_json, content)
            if filepath: return await asyncio.to_thread(pd.read_json, filepath)
        elif source_type == "parquet":
            import io
            if content: return await asyncio.to_thread(pd.read_parquet, io.BytesIO(content))
            if filepath: return await asyncio.to_thread(pd.read_parquet, filepath)
        elif source_type == "sql":
            return await self._load_sql(payload)
        elif source_type == "api":
            return await self._load_api(payload)
        raise ValueError(f"Unsupported source type or missing content: {source_type}")
    ...
    async def _load_api(self, payload: Dict) -> pd.DataFrame:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            resp = await client.request(method=cfg.get("method","GET"), url=cfg["url"],
    ...
    def _validate(self, df: pd.DataFrame, cols: list) -> Dict[str, Any]:
        ...
        for c in cols:
            p = c["null_pct"]
            if p > 50: issues.append(f"'{c['name']}' is {p:.1f}% null")
            elif p > 20: warnings.append(f"'{c['name']}' is {p:.1f}% null")
        dups = int(df.duplicated().sum())
        if dups / max(len(df), 1) > 0.1: issues.append(f"{dups} duplicate rows ({dups/len(df)*100:.1f}%)")
        elif dups: warnings.append(f"{dups} duplicate rows")
        return {"is_valid": not issues, "issues": issues, "warnings": warnings, "duplicate_rows": dups}
...
class UnderstandingAgent(BaseAgent):
    ...
    async def run(self, payload: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
        ...
        if not schema or not rows:
            raise ValueError(f"Schema/sample not found for {dataset_id}. Run ingestion first.")
        ...
        if payload.get("run_time_series") and time_col:
            val_cols = payload.get("value_columns") or [c["name"] for c in schema["columns"] if c["inferred_type"] == "numeric"][:3]
            time_series = await self._time_series(df, time_col, val_cols)

        contribution = {}
        if payload.get("run_contribution") and payload.get("dimension_col") and payload.get("metric_col"):
    ...
    def _missing_report(self, df: pd.DataFrame) -> Dict:
        ...
        return {
            "per_column": {c: {"count": int(m[c]), "pct": float(p[c])} for c in df.columns if m[c] > 0},
    ...
    def _distribution(self, df: pd.DataFrame, schema: Dict) -> Dict:
        ...
        for col in schema.get("columns", []):
            ...
            if ct == "numeric":
                ...
                try: sk, ku = float(sp.skew(s)), float(sp.kurtosis(s))
                except Exception: sk, ku = 0.0, 0.0
                counts, edges = np.histogram(s, bins=min(20, s.nunique()))
                result[name] = {"skewness": round(sk,4), "kurtosis": round(ku,4),
                                "histogram": {"counts": counts.tolist(), "edges": [round(e,4) for e in edges.tolist()]},
                                "p5": float(s.quantile(.05)), "p25": float(s.quantile(.25)),
                                "p50": float(s.quantile(.50)), "p75": float(s.quantile(.75)), "p95": float(s.quantile(.95))}
            elif ct == "categorical":
                vc = df[name].value_counts()
                result[name] = {"top_values": vc.head(10).to_dict(), "unique_count": int(df[name].nunique()),
    ...
    def _anomalies(self, df: pd.DataFrame, schema: Dict) -> List[Dict]:
        anomalies = []
        for col in schema.get("columns", []):
            if col["inferred_type"] != "numeric" or col["name"] not in df.columns: continue
            s = df[col["name"]].dropna()
            if len(s) < 10: continue
            for method, fn in [("zscore", detect_anomalies_zscore), ("iqr", detect_anomalies_iqr)]:
                try:
                    idxs, thresh = fn(s)
                    if idxs:
                        anomalies.append({"column": col["name"], "method": method,
                                          "anomaly_count": len(idxs),
                                          "anomaly_pct": round(len(idxs)/len(s)*100, 2),
                                          "anomaly_indices": idxs[:20], "threshold": thresh})
                except Exception: pass
        return anomalies

    async def _time_series(self, df: pd.DataFrame, time_col: str, val_cols: List[str]) -> Dict:
        result: Dict[str, Any] = {"time_column": time_col, "series": {}}
        for col in val_cols:
            if col not in df.columns: continue
            try:
                decomp = await asyncio.to_thread(time_series_decompose, df, time_col, col)
                ...
                result["series"][col] = {"decomposition": decomp, "moving_averages": ma, "forecast_14d": forecast}
    ...
    def _top_correlations(self, corr: Dict) -> str:
        ...
        return "; ".join(f"'{a}' ↔ '{b}': {'positive' if v>0 else 'negative'} ({v:.2f})" for a,b,v in pairs[:5]) or "No significant correlations."

    def _warnings(self, missing: Dict, quality: float) -> List[str]:
        w = []
        for col, info in missing.get("per_column", {}).items():
            if info["pct"] > 30: w.append(f"'{col}' has {info['pct']}% missing values.")
        if quality < 60: w.append(f"Data quality score is low ({quality}/100).")
        return w
...
class FeatureAgent(BaseAgent):
    ...
    async def run(self, payload: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
        ...
        return {"dataset_id": dataset_id, "transformations": result["transformations"], "new_columns": result["new_columns"]}
        ...
        for col in schema.get("columns", []):
            ...
            if ct == "datetime":
                ...
                if not df[name].isna().all():
                    ...
                    transformations.append(f"Extracted year/month/dayofweek from '{name}'")
            elif ct == "numeric":
                s = df[name].dropna()
            ...
            elif ct == "categorical" and col.get("cardinality") in ("low","medium") and col["unique_count"] <= 50:
                from sklearn.preprocessing import LabelEncoder
                non_null = df[name].dropna()
                if len(non_null):
                    le = LabelEncoder()
                    df.loc[non_null.index, f"{name}_enc"] = le.fit_transform(non_null.astype(str))
                    transformations.append(f"Label encoded '{name}'")
        return {"rows": sanitize_rows(df.head(100).to_dict("records")), "transformations": transformations, "new_columns": len(df.columns)}
...
class InsightAgent(BaseAgent):
    ...
    async def run(self, payload: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
        ...
        schema = await self._cache.get_schema(dataset_id)
        if not eda or not schema:
            raise ValueError(f"EDA/schema missing for {dataset_id}. Run process-data first.")
        ...
        req = LLMRequest(
            prompt=prompt, system_prompt=INSIGHT_SYSTEM,
            provider=LLMProvider(payload["llm_provider"]) if payload.get("llm_provider") else None,
            model_override=payload.get("model_override"),
            mode=LLMMode.ADVANCED, temperature=settings.LLM_TEMPERATURE,
            max_tokens=settings.LLM_MAX_TOKENS,
        )
        resp = await self._llm.complete(req)

        try:
            data = LLMService.extract_json(resp.content)
        except Exception:
            data = {"insights": [], "executive_summary": resp.content[:1000]}
...
class QueryAgent(BaseAgent):
    ...
    async def run(self, payload: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
        ...
        cache_key = hashlib.sha256(f"{dataset_id}:{question.strip().lower()}".encode()).hexdigest()
        if use_cache:
        ...
        schema_json = json.dumps({"columns": [{"name": c["name"], "type": c["inferred_type"]} for c in schema.get("columns", [])]}, indent=2)
        sample_str = json.dumps(rows[:5], default=str)

        req = LLMRequest(
            prompt=build_query_prompt(schema_json, sample_str, rag_context, question, intent, payload.get("output_format","table")),
            system_prompt=QUERY_SYSTEM,
            provider=LLMProvider(payload["llm_provider"]) if payload.get("llm_provider") else None,
            model_override=payload.get("model_override"),
            mode=LLMMode.ADVANCED, temperature=0.1, max_tokens=2048,
        )
        resp = await self._llm.complete(req)

        try:
            plan = LLMService.extract_json(resp.content)
        except Exception:
            raise ValueError(f"LLM returned unparseable response: {resp.content[:300]}")
        ...
        execution = await asyncio.to_thread(self._exec_code, plan.get("generated_code",""), df)

        result = {
            "dataset_id": dataset_id, "question": question,
            "plan": {"generated_code": plan.get("generated_code",""),
    ...
    def _exec_code(self, code: str, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        rdf = rdf.head(500)
        return {"columns": list(rdf.columns), "rows": sanitize_rows(rdf.to_dict("records")), "row_count": len(rdf)}

    def _detect_intent(self, q: str) -> str:
        q = q.lower()
        if any(w in q for w in ["trend","over time","monthly","daily","weekly"]): return "trend"
        if any(w in q for w in ["compare","vs","versus","difference"]): return "comparison"
        if any(w in q for w in ["distribution","spread","histogram"]): return "distribution"
        if any(w in q for w in ["top","bottom","highest","lowest","rank"]): return "ranking"
        if any(w in q for w in ["correlation","relationship","impact"]): return "correlation"
        if any(w in q for w in ["sum","total","count","average","mean"]): return "aggregation"
        return "general"
...
class VisualizationAgent(BaseAgent):
    ...
    async def run(self, payload: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
        ...
        if not schema: raise ValueError(f"No schema for {dataset_id}")

        df = pd.DataFrame(rows or [])
        cols = schema.get("columns", [])
        charts = await asyncio.to_thread(self._generate_all, df, cols, eda or {}, payload.get("question",""))
    ...
    def _generate_all(self, df: pd.DataFrame, cols: List[Dict], eda: Dict, question: str) -> List[Dict]:
        charts = []
        from backend.services.chart_engine import _nums, _cats, _dts
        num, cat, dts = _nums(cols), _cats(cols), _dts(cols)

        combos = []
        if dts and num: combos.append(("line", {}))
        if cat and num: combos.append(("bar", {}))
        if num: combos.append(("histogram", {}))
        if len(num) >= 2: combos.append(("scatter", {}))
        if cat and num and any(df[c].nunique() <= 8 for c in cat): combos.append(("pie", {}))
        if len(num) >= 2: combos.append(("heatmap", {}))
        if len(num) >= 3: combos.append(("radar", {}))

        intents = {
            "line": "trend", "bar": "comparison", "histogram": "distribution",
            "scatter": "correlation", "pie": "part-whole", "heatmap": "correlation", "radar": "comparison",
        }
        for ct, _ in combos:
            try:
                cfg = self._engine.generate(df, cols, requested_chart=ct,
                                            title="", intent=intents.get(ct,"general"))
    ...
    def _kpis(self, df: pd.DataFrame, cols: List[Dict]) -> List[Dict]:
        ...
        for col in _nums(cols)[:4]:
            ...
            kpis.append({"column": col, "sum": _safe(s.sum()), "mean": round(float(s.mean()),4),
                         "min": _safe(s.min()), "max": _safe(s.max()), "count": int(len(s))})
        return kpis
...
class ReportAgent(BaseAgent):
    ...
    async def run(self, payload: Dict[str, Any], correlation_id: str) -> Dict[str, Any]:
        ...
        if not schema: raise ValueError(f"Schema not found for {dataset_id}")
        ...
        prompt = build_report_prompt(
            ...
            sections=payload.get("sections", ["executive_summary","eda","insights","anomalies","recommendations"]),
        )

        req = LLMRequest(
            prompt=prompt, system_prompt=REPORT_SYSTEM,
            provider=LLMProvider(payload["llm_provider"]) if payload.get("llm_provider") else None,
            model_override=payload.get("model_override"),
            mode=LLMMode.ADVANCED, temperature=settings.LLM_TEMPERATURE,
        ...
        try:
            ...
            data = {"sections": [{"section_id": "main", "title": "Report", "content": resp.content, "order": 1}]}
        ...
        return {
            ...
            "charts": charts_data.get("charts", [])[:5],
            "provider": resp.provider.value,
            "model": resp.model,
        }

    def _eda_summary(self, eda: Dict) -> str:
        return (f"Quality: {eda.get('quality_score','N/A')}/100. "
    ...
    def _insights_summary(self, insights: List[Dict]) -> str:
        return "\n".join(f"[{i.get('category','').upper()}] {i.get('title','')}: {i.get('description','')[:200]}" for i in insights[:5]) or "No insights generated."

    def _anomaly_summary(self, anomalies: List[Dict]) -> str:
        return "\n".join(f"'{a.get('column')}': {a.get('anomaly_count')} anomalies ({a.get('anomaly_pct')}%) via {a.get('method')}." for a in anomalies[:5]) or "No anomalies."
