...
class DemoSeedRequest(BaseModel):
    kind: str = Field(..., description="Demo dataset kind, see /datasets/demo-catalogue")


@advanced_router.post("/datasets/seed-demo/{kind}")
async def seed_demo_kind(kind: str, storage: StorageService = Depends(_storage)) -> Dict[str, Any]:
    try:
...
async def dataset_stats(
    ...
    quality_score = round(100 * (1 - (missing_cells / total_cells) if total_cells else 0), 1)
...
async def correlations(
    ...
    return {
        ...
        "pairs": AdvancedAnalytics.deep_correlation(df, corr.columns[0], corr.columns.tolist()).get("correlations", [])[:20],
    }
...
async def trends(
    ...
    df = await _load_df(dataset_id, storage)
    date_cols = [c for c in df.columns if "date" in c.lower() or "ts" in c.lower() or "time" in c.lower()]
    numeric_cols = df.select_dtypes(include=np.number).columns.tolist()
...
async def forecast(
    ...
    history = [
        {"date": str(ts_val.date() if hasattr(ts_val, "date") else ts_val), "value": round(float(v), 4)}
    ...
    for i, y in enumerate(future_y, start=1):
        ...
        future.append({"date": str(future_ts.date() if hasattr(future_ts, "date") else future_ts), "value": round(float(y), 4)})
...
async def anomalies(
    ...
    for idx, (value, score) in enumerate(zip(s[hits].values, z[hits].values)):
        points.append({"index": int(s[hits].index[idx]), "value": float(value), "z_score": round(float(score), 3)})
    return {
        "column": column,
        "count": int(hits.sum()),
...
async def cleaning_preview(
    ...
    df = await _load_df(dataset_id, storage)
    ops = [CleaningOp(op=s.op, columns=s.columns, params=s.params) for s in body.operations]
...
async def cleaning_suggestions(
    ...
    for col, pct in null_pct.sort_values(ascending=False).head(5).items():
        if pct <= 0:
            continue
        if pct > 40:
            suggestions.append({"op": "drop_columns", "columns": [col], "label": f"Drop '{col}' ({pct:.0f}% missing)"})
        elif pd.api.types.is_numeric_dtype(df[col]):
            suggestions.append({"op": "fill_missing", "columns": [col], "params": {"strategy": "median"},
                                "label": f"Fill '{col}' with median ({pct:.0f}% missing)"})
        else:
            suggestions.append({"op": "fill_missing", "columns": [col], "params": {"strategy": "mode"},
                                "label": f"Fill '{col}' with mode ({pct:.0f}% missing)"})

    # Duplicates
    dup_rows = int(df.duplicated().sum())
    if dup_rows > 0:
        suggestions.append({"op": "drop_duplicates", "columns": None,
                            "label": f"Drop {dup_rows} duplicate rows"})

    # String trimming
    text_cols = df.select_dtypes(include="object").columns.tolist()
    if text_cols:
        suggestions.append({"op": "trim_strings", "columns": text_cols[:5],
    ...
    for col in numeric_cols[:3]:
        ...
        if outlier_count > 0:
            suggestions.append({"op": "remove_outliers", "columns": [col],
                                "params": {"method": "iqr"},
