...
from backend.services.guardrails import InputGuardrails
...
async def clear_system(
    ...
    try:
        ...
        raise HTTPException(500, f"Failed to clear system: {e}")
...
async def list_chart_types():
    ...
    return {
        "chart_types": [
            {"id": ct.value, "label": ct.value.replace("_", " ").title()}
            for ct in ChartType
        ]
    }
...
async def list_datasets(
    ...
    for d in raw:
        ...
        if not existing or (
            entry.get("_has_schema") and not existing.get("_has_schema")
        ):
            seen_names[key] = entry
    enriched = [
        {k: v for k, v in e.items() if k != "_has_schema"} for e in seen_names.values()
    ]
...
async def preview_dataset(
    dataset_id: str, n: int = 20, cache: CacheService = Depends(get_cache)
):
    schema = await cache.get_schema(dataset_id)
    rows = await cache.get_sample(dataset_id)
    if not schema:
...
async def run_analytics(
    dataset_id: str, body: AnalyticsReq, cache: CacheService = Depends(get_cache)
):
    df, schema = await _get_df_and_schema(dataset_id, cache)
    num_cols = [
        c["name"]
        for c in schema.get("columns", [])
        if c.get("inferred_type") == "numeric"
    ]
    cfg = body.config
    ...
    if body.analysis_type == "kpis":
        return {
            "dataset_id": dataset_id,
            "kpis": _analytics.compute_kpis(df, num_cols, cfg.get("date_col")),
        }
    elif body.analysis_type == "cohort":
        return _analytics.cohort_analysis(
            df, cfg["date_col"], cfg["user_col"], cfg["value_col"]
        )
    elif body.analysis_type == "funnel":
        return _analytics.funnel_analysis(
            df, cfg["stage_col"], cfg["value_col"], cfg.get("stage_order")
        )
    elif body.analysis_type == "group_aggregate":
        return _analytics.group_aggregate(
            df,
            cfg["group_cols"],
            cfg["agg_col"],
        ...
        return {
            "dataset_id": dataset_id,
            "outliers": _analytics.outlier_summary(df, num_cols),
        }
...
async def compare_datasets(body: CompareReq, cache: CacheService = Depends(get_cache)):
    ...
    result: Dict[str, Any] = {
        "dataset_a": {
            "id": body.dataset_id_a,
            "rows": len(df_a),
            "cols": len(df_a.columns),
        },
        "dataset_b": {
            "id": body.dataset_id_b,
            "rows": len(df_b),
...
async def generate_insights(
    dataset_id: str, body: InsightReq, cache: CacheService = Depends(get_cache)
):
    if not await cache.get_schema(dataset_id):
        raise HTTPException(404, f"Dataset '{dataset_id}' not found.")
    task = task_insights.apply_async(
        kwargs={
            "payload": {
                "dataset_id": dataset_id,
...
async def chat(body: ChatReq, llm: LLMService = Depends(get_llm)):
    ...
    return {
        ...
        if hasattr(resp.provider, "value")
...
async def validate_chart(
    body: ChartValidateReq, cache: CacheService = Depends(get_cache)
):
...
async def generate_chart(
    body: ChartGenerateReq, cache: CacheService = Depends(get_cache)
):
    df, schema = await _get_df_and_schema(body.dataset_id, cache)
    if body.chart_type:
        ok, msg = InputGuardrails.validate_chart_type(body.chart_type)
    ...
    config = _chart_engine.generate(
        ...
        requested_chart=body.chart_type,
        title=body.title or "",
        intent=body.intent,
    ...
    await cache.cache_chart_config(
        f"{body.dataset_id}:{body.chart_type or 'auto'}", config
    )
    return config
...
async def generate_dashboard(
    ...
    if not await cache.get_schema(dataset_id):
        raise HTTPException(404, f"Dataset '{dataset_id}' not found.")
    task = task_dashboard.apply_async(
        kwargs={
            "payload": {
                "dataset_id": dataset_id,
                "focus_columns": body.focus_columns,
...
async def generate_react_layout(
    body: ReactDashboardReq,
    cache: CacheService = Depends(get_cache),
    llm: LLMService = Depends(get_llm),
):
    ...
    chart_summary = json.dumps(
        [
            ...
            for c in charts_data.get("charts", [])[:10]
        ]
    )
    kpi_summary = json.dumps(charts_data.get("kpis", [])[:6])

    from backend.prompts.react_prompts import (
    ...
    req = LLMRequest(
        prompt=prompt,
        system_prompt=REACT_DASHBOARD_SYSTEM,
        mode=LLMMode.FAST,
        temperature=0.2,
        max_tokens=2000,
        provider=None,
...
async def generate_react_component(
    body: ReactComponentReq, llm: LLMService = Depends(get_llm)
):
    ok, msg = InputGuardrails.validate_chart_type(body.chart_type)
    if not ok:
        raise HTTPException(400, msg)
    from backend.prompts.react_prompts import (
        REACT_COMPONENT_SYSTEM,
    ...
    req = LLMRequest(
        prompt=prompt,
        system_prompt=REACT_COMPONENT_SYSTEM,
...
async def get_api_integration_guide():
    ...
    endpoints = [
        {
            "method": "POST",
            "path": "/api/v1/upload-data",
            "description": "Upload CSV/JSON/Parquet",
        },
        {
            "method": "POST",
            "path": "/api/v1/process-data/{dataset_id}",
            "description": "Run EDA",
        },
        {
            "method": "POST",
            "path": "/api/v1/generate-insights/{dataset_id}",
            "description": "Generate insights",
        },
        {
            "method": "POST",
            "path": "/api/v1/query/{dataset_id}",
            "description": "NL query",
        },
        {
            "method": "POST",
            "path": "/api/v1/chart/validate",
            "description": "Validate chart type",
        },
        {
            "method": "POST",
            "path": "/api/v1/chart/generate",
            "description": "Generate chart config",
        },
        {
            "method": "GET",
            "path": "/api/v1/chart/recommend/{dataset_id}",
            "description": "Recommend charts",
        },
        {
            "method": "POST",
            "path": "/api/v1/dashboard/{dataset_id}",
            "description": "Generate dashboard",
        },
        {
            "method": "POST",
            "path": "/api/v1/analytics/{dataset_id}",
...
async def export_insights_markdown(
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename={dataset_id}_insights.md"
        },
    )
...
async def list_provider_status(llm: LLMService = Depends(get_llm)):
    ...
    provider_keys = {
        "openai": _s.OPENAI_API_KEY or "",
        "anthropic": _s.ANTHROPIC_API_KEY or "",
        "gemini": _s.GEMINI_API_KEY or "",
        "groq": _s.GROQ_API_KEY or "",
    }
    default_key = provider_keys.get(_s.DEFAULT_LLM_PROVIDER, "")
    fallback_key = next((v for v in provider_keys.values() if v), "")
    key = default_key or fallback_key
