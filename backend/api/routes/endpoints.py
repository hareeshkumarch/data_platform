...
async def seed_demo_dataset(
    ...
    try:
        ...
        raise HTTPException(500, f"Demo file not found or unreadable: {e}")
...
async def chat(body: ChatReq, llm: LLMService = Depends(get_llm)):
    req = LLMRequest(prompt=body.prompt, mode=LLMMode.FAST)
    resp = await llm.complete(req)
    return {"reply": resp.content}

@router.get("/chat-stream")
async def chat_stream(prompt: str, llm: LLMService = Depends(get_llm)):
    req = LLMRequest(prompt=prompt, mode=LLMMode.FAST)
...
async def task_status(task_id: str):
    ...
    resp: Dict[str, Any] = {"task_id": task_id, "status": r.state.lower()}
    if r.state == "SUCCESS":
        resp.update({"result": r.result, "progress": 100.0})
...
async def system_stats(llm: LLMService = Depends(get_llm)):
    ...
    calls = get_val("llm_calls_total")
    tokens = get_val("llm_tokens_total")
    
    # Calculate success rate
    success_calls = 0.0
    for m in REGISTRY.collect():
        if m.name == "llm_calls_total":
            for s in m.samples:
                if s.labels.get("status") == "success":
                    success_calls += s.value
    
    rate = (success_calls / calls * 100) if calls > 0 else 100.0
