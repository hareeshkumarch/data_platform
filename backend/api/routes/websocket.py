...
def _build_agent_summaries(result_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if ing_err:
        summaries.append(
            {
                ...
                "status": "error",
                "headline": "Data ingestion failed",
                "details": [str(ing_err)],
            }
        )
    elif ing:
        ...
        numeric_count = sum(
            1
            for c in columns
            if isinstance(c, dict) and c.get("inferred_type") == "numeric"
        )
        cat_count = sum(
            1
            for c in columns
            if isinstance(c, dict) and c.get("inferred_type") == "categorical"
        )
        quality = ing.get("schema", {}).get("quality_score", 0)
        details = [f"Loaded **{rows:,}** rows × **{cols}** columns"]
        if numeric_count or cat_count:
            details.append(
                f"**{numeric_count}** numeric, **{cat_count}** categorical, **{cols - numeric_count - cat_count}** other columns detected"
            )
        if quality:
        ...
        summaries.append(
            {
                ...
                "agentName": "Ingestion Agent",
                "status": "success",
                "headline": f"Validated {rows:,} rows across {cols} columns",
                "details": details,
            }
        )
    else:
    ...
    if und_err:
        summaries.append(
            {
                ...
                "status": "error",
                "headline": "Profiling encountered an error",
                "details": [str(und_err)],
            }
        )
    elif und:
        ...
        if anom_count > 0:
            ...
            for a in anomalies[:3]:
                if isinstance(a, dict):
                    ...
                    details.append(
                        f"• **{col}**: {count} outliers ({pct}%) via {method}"
                    )
        ...
        if per_col:
            top_missing = sorted(
                per_col.items(),
                key=lambda x: -x[1].get("pct", 0) if isinstance(x[1], dict) else 0,
            )[:3]
            items = [
                f"**{k}** ({v.get('pct', 0)}%)"
                for k, v in top_missing
                if isinstance(v, dict) and v.get("pct", 0) > 0
            ]
            if items:
        ...
        summaries.append(
            {
                ...
                "agentName": "Understanding Agent",
                "status": "success",
                "headline": f"Quality {score}/100 — {anom_count} anomalies detected",
                "details": details,
            }
        )
    else:
        ...
        summaries.append(
            {
                ...
                "status": "error",
                "headline": "Feature engineering failed",
                "details": [str(feat_err)],
            }
        )
    elif feat:
        ...
        summaries.append(
            {
                ...
                "agentName": "Feature Agent",
                "status": "success",
                "headline": f"Applied {feat_count} transformations ({new_cols} columns)",
                "details": details,
            }
        )
    else:
    ...
    if ins_err:
        summaries.append(
            {
                ...
                "status": "error",
                "headline": "Insight detection failed",
                "details": [str(ins_err)],
            }
        )
        ...
        headline = (
            ...
            else f"Surfaced {ins_count} key insights"
        )
        ...
        summaries.append(
            {
                ...
                "agentName": "Visualization Agent",
                "status": "success",
                "headline": f"Rendered {chart_count} interactive charts",
                "details": details,
            }
        )
    else:
        summaries.append(
            {
                ...
                "agentName": "Visualization Agent",
                "status": "skipped",
                "headline": "No charts generated",
                "details": ["No suitable chart types for this data configuration"],
            }
        )
    ...
    if rep_err:
        summaries.append(
            {
                ...
                "status": "error",
                "headline": "Report compilation failed",
                "details": [str(rep_err)],
            }
        )
    elif rep:
        ...
        headline = (
            exec_headline
            if exec_headline
            else f"Executive report compiled — {sec_count} sections"
        )
    ...
    if eva_err:
        ...
        headline = (
            f"Quality score: {score}/10 — {eva.get('refinement_instruction', 'Ready')}"
        )
        ...
        if not satisfied:
            details.append(
                f"⚠️ **Refinement suggested**: {eva.get('refinement_instruction')}"
            )
        summaries.append(
            {
                "agentId": "evaluator",
    ...
    async def pipeline_ws(websocket: WebSocket) -> None:
        ...
        try:
            ...
            if result.success and result.data:
                ...
                reply = (
                    f"Pipeline encountered an issue: {result.error or 'Unknown error'}"
                )
            ...
            for i, (sid, t_start) in enumerate(timing_entries):
                t_end = (
                    timing_entries[i + 1][1]
                    if i + 1 < len(timing_entries)
            ...
            await websocket.send_json(
                {
                    ...
                    "charts": result.data.get("visualization", {}).get("charts", [])
                    if result.data
                    else [],
