...
Model Tiers: "fast" (for basic aggregations and simple questions), "advanced" (for complex ML, statistical modelling, or reasoning questions)
...
Classify the intent, the required query type, and the model tier.
...
SCHEMA_FILTER_SYSTEM = """You are a dataset schema analyzer.
Your job is to look at a user's analytical question and a list of available columns, and select ONLY the columns that are likely to be required to answer the question.
...
Select the exact names of the columns needed to answer the question. Only include columns that actually exist in the list above.
...
INSIGHT_CRITIC_SYSTEM = """You are a critic validation engine.
Your job is to read an insight generated by another AI and compare it against the raw ground-truth statistics.
If the insight invents numbers, exaggerates, or hallucinates, you must reject it.
...
Does this insight accurately reflect the ground truth statistics? Output confidence score from 0.0 to 1.0.

Return this exact JSON:
{{
  "is_valid": <bool>,
  "confidence": <float>,
  "explanation": "<1 short sentence explaining why it is valid or invalid>"
}}"""
