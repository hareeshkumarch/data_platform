        ...
        try:
            return (
                ...
                .apply(lambda g: g.sample(
                    min(len(g), max(1, int(settings.SAMPLE_SIZE * len(g) / len(df)))),
                    random_state=42,
                ))
...
def compute_summary_stats(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if not num.empty:
        result["numeric"] = _convert(num.describe(percentiles=[0.25, 0.5, 0.75, 0.9]).to_dict())
    cat = df.select_dtypes(["object", "category"])
    if not cat.empty:
        result["categorical"] = {
            c: {"top_values": _convert(df[c].value_counts().head(10).to_dict()),
...
def detect_anomalies_zscore(series: pd.Series, threshold: float = 3.0) -> Tuple[List[int], float]:
...
def time_series_decompose(df: pd.DataFrame, time_col: str, value_col: str, period: int = 7) -> Dict[str, Any]:
    ...
    return {
        ...
        "observed": _to_list(ts), "trend": _to_list(trend),
        "seasonal": _to_list(seasonal), "residual": _to_list(residual),
    }


def compute_moving_averages(df: pd.DataFrame, time_col: str, value_col: str) -> Dict[str, Any]:
    ts = df[[time_col, value_col]].copy()
    ts[time_col] = pd.to_datetime(ts[time_col], errors="coerce")
    ts = ts.dropna().sort_values(time_col).set_index(time_col)
    result: Dict[str, Any] = {"dates": ts.index.astype(str).tolist(), "raw": _to_list(ts[value_col])}
    for w in [7, 14, 30]:
        if len(ts) >= w:
...
def contribution_analysis(df: pd.DataFrame, dim_col: str, metric_col: str) -> List[Dict[str, Any]]:
