...
def _sales(rows: int = 420, seed: int = 7) -> pd.DataFrame:
    ...
    for i in range(rows):
        ...
        base = 35_000 if segment == "Enterprise" else 12_000 if segment == "SMB" else 6_000
...
def _ecommerce(rows: int = 600, seed: int = 11) -> pd.DataFrame:
    _seed(seed)
    categories = ["Electronics", "Home", "Fashion", "Beauty", "Sports", "Books", "Grocery"]
...
def _marketing(rows: int = 280, seed: int = 19) -> pd.DataFrame:
    ...
    campaigns = ["winter_launch", "spring_reboot", "retention_30d", "enterprise_abm",
                 "student_deal", "holiday_bundle"]
    start = date(2024, 1, 1)
    records = []
    for i in range(rows):
...
def _finance(rows: int = 520, seed: int = 23) -> pd.DataFrame:
    _seed(seed)
    tickers = ["AAPL", "MSFT", "NVDA", "GOOGL", "META", "AMZN", "TSLA", "NFLX", "AMD", "CRM"]
    records = []
    start = date(2024, 1, 2)
    for ticker in tickers:
        ...
        for i in range(rows // len(tickers)):
            ...
            records.append({
                ...
                "volume": volume,
                "sector": random.choice(["Tech", "Tech", "Media", "Retail", "Automotive"]),
            })
    return pd.DataFrame(records)
...
def _hr(rows: int = 300, seed: int = 29) -> pd.DataFrame:
    ...
    for i in range(rows):
        ...
        base = {"IC1": 55, "IC2": 75, "IC3": 105, "IC4": 140, "Manager": 160, "Director": 210}[level]
        ...
        records.append({
            ...
            "department": dept,
            "level": level,
            "location": random.choice(["NYC", "SFO", "LON", "BLR", "BER", "Remote"]),
...
def _customers(rows: int = 500, seed: int = 31) -> pd.DataFrame:
    ...
    for i in range(rows):
        ...
        mrr = {"free": 0, "starter": 29, "pro": 99, "business": 299, "enterprise": 1200}[plan]
        active = random.random() < 0.78
        records.append({
            "customer_id": f"CUST-{10000 + i}",
            "signup_date": signup.isoformat(),
            "plan": plan,
            "country": random.choice(["US", "UK", "DE", "IN", "BR", "CA", "AU", "FR"]),
            "industry": random.choice(["SaaS", "Retail", "Fintech", "Health", "Edu", "Media"]),
            "mrr_usd": mrr + (random.randint(0, 350) if plan == "enterprise" else 0),
...
def _iot(rows: int = 1000, seed: int = 37) -> pd.DataFrame:
    ...
    for i in range(rows):
        ...
        records.append({
            ...
            "temperature_c": round(base_temp + np.random.normal(0, 0.9) + anomaly * 8, 2),
            "humidity_pct": round(45 + np.random.normal(0, 6), 1),
            "pressure_kpa": round(101.3 + np.random.normal(0, 0.35), 2),
            "vibration_g": round(abs(np.random.normal(0.12, 0.04)) + anomaly * 0.5, 4),
