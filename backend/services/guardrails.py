...
class InputGuardrails:
    ...
    @staticmethod
    def validate_file_upload(filename: str, size_bytes: int, content_type: str = "") -> Tuple[bool, str]:
        ext = filename.rsplit(".", 1)[-1].lower() if "." in filename else ""
        if ext not in _ALLOWED_EXTENSIONS:
            return False, f"File type '.{ext}' not supported. Allowed: {', '.join(_ALLOWED_EXTENSIONS)}"
        if size_bytes > _MAX_UPLOAD_BYTES:
            return False, f"File size {size_bytes/1024/1024:.1f}MB exceeds 500MB limit."
    ...
    def validate_sql_query(query: str) -> Tuple[bool, str]:
        if _DANGEROUS_SQL.search(query):
            return False, "SQL query contains disallowed statements (DROP, DELETE, ALTER, etc.)."
        if len(query) > 5000:
    ...
    def validate_connection_string(conn_str: str) -> Tuple[bool, str]:
        allowed_schemes = {"postgresql", "mysql", "sqlite", "mssql", "postgresql+psycopg2"}
        scheme = conn_str.split("://")[0].lower() if "://" in conn_str else ""
        if scheme not in allowed_schemes:
            return False, f"Database scheme '{scheme}' not allowed. Supported: {allowed_schemes}"
        return True, ""
...
class RateLimiter:
    ...
    async def check(self, endpoint: str, identifier: str) -> Tuple[bool, str]:
        ...
        if not allowed:
            return False, f"Rate limit exceeded for {endpoint}. Limit: {limit} req/{window}s."
        return True, ""
