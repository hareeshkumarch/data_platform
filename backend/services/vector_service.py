    ...
    async def embed(self, texts: List[str]) -> List[List[float]]:
        if self._gemini:
            try:
                ...
                logger.error("Gemini embedding failed, falling back to hash", error=str(e))
...
class FAISSStore:
    ...
    def _persist(self, dataset_id: str):
        import faiss
        faiss.write_index(self._indexes[dataset_id], str(self._dir / f"{dataset_id}.index"))
    ...
    async def add(self, dataset_id: str, docs: List[RAGDocument]):
        def _run():
            import faiss
            vecs = np.array([d.embedding for d in docs if d.embedding], dtype=np.float32)
            if not len(vecs):
                return
    ...
    async def search(self, dataset_id: str, query_vec: List[float], top_k: int = 5) -> List[Tuple[RAGDocument, float]]:
        def _run():
            import faiss
            if dataset_id not in self._indexes:
            ...
            return [(docs[i], float(s)) for s, i in zip(scores[0], idxs[0]) if 0 <= i < len(docs)]
...
class VectorService:
    ...
    async def index_dataset(self, dataset_id: str, schema: Dict, stats: Dict, insights: List[str] = None):
        docs: List[RAGDocument] = []

        docs.append(RAGDocument(
            dataset_id=dataset_id,
            content=f"Dataset schema: {json.dumps(schema, indent=2)[:2000]}",
            doc_type="schema", metadata={"type": "schema"},
        ))
        for col in schema.get("columns", []):
            docs.append(RAGDocument(
                dataset_id=dataset_id,
                content=(f"Column '{col['name']}': type={col['inferred_type']}, "
                         f"unique={col['unique_count']}, nulls={col['null_pct']}%, "
                         f"samples={col.get('sample_values', [])[:3]}"),
                doc_type="column", metadata={"column": col["name"]},
            ))
        if stats:
            docs.append(RAGDocument(
                dataset_id=dataset_id,
                content=f"Statistics: {json.dumps(stats)[:2000]}",
                doc_type="stats", metadata={},
            ))
        for i, text in enumerate(insights or []):
            docs.append(RAGDocument(
                dataset_id=dataset_id, content=text, doc_type="insight", metadata={"index": i},
            ))
    ...
    async def retrieve(self, dataset_id: str, query: str, top_k: int = 5) -> str:
        ...
        for doc, vec_score in results:
            kw = len(q_tokens & set(doc.content.lower().split())) / max(len(q_tokens), 1)
