...
class AdvancedAnalytics:
    ...
    def _growth_rate(
        ...
        return _growth_rate(df, date_col, value_col)
    ...
    def funnel_analysis(
        ...
        try:
            ...
            return {
                ...
                "totals": _safe_list(grouped["total"].tolist()),
    ...
    def profile_dataset(df: pd.DataFrame, schema_cols: List[Dict]) -> Dict[str, Any]:
        ...
        for col_meta in schema_cols:
            ...
            if col_meta.get("inferred_type") == "numeric":
                ns = s.dropna()
                if len(ns):
                    profile.update(compute_distribution(ns))
                    profile.update(
                        {
                            "mean": _safe(ns.mean()),
                            "std": _safe(ns.std()),
    ...
    def distribution_analysis(
        ...
        for col in numeric_cols[:12]:
            ...
            sample = (
                s.sample(min(len(s), 5000), random_state=42) if len(s) > 5000 else s
            )
            try:
            ...
            _mean = float(s.mean())
            cv = (
                abs(float(s.std()) / _mean) * 100
                if _mean != 0 and math.isfinite(_mean)
                else 0
            )
                ...
                suggestions.append(
                    {
                        "column": col,
                        "action": "drop_column",
                        "reason": f"{null_pct}% missing — consider dropping",
                    }
                )
            elif null_pct > 5:
                issues.append("moderate_missing")
                fill = (
                    "median" if col_meta.get("inferred_type") == "numeric" else "mode"
                )
                suggestions.append(
                    {
                        "column": col,
                        "action": f"fill_{fill}",
                        "reason": f"{null_pct}% missing — impute with {fill}",
                    }
                )
            if s.nunique() <= 1:
                ...
                suggestions.append(
                    {
                        "column": col,
                        "action": "drop_column",
                        "reason": "Column is constant — adds no information",
                    }
                )
            ...
            suggestions.append(
                {
                    "column": "_rows_",
                    "action": "dedup",
                    "reason": f"{dup_rows} duplicate rows found",
                }
            )
    ...
    def ts_decomposition(
        ...
        try:
            ...
            seasonal = detrended.groupby(np.arange(len(detrended)) % period).transform(
                "mean"
            )
            ...
            seasonality_strength = round(
                max(0, 1 - var_resid / (var_deseason + 1e-10)), 3
            )
    ...
    def seasonal_anomalies(
        ...
        try:
            ...
            robust_sigma = (
                float((residual - residual.median()).abs().median() * 1.4826) or sigma
            )
                ...
                for i in range(contingency.shape[0]):
                    for j in range(contingency.shape[1]):
                        if contingency[i, j] > 0 and px[i] > 0 and py[j] > 0:
                            mi += contingency[i, j] * math.log2(
                                contingency[i, j] / (px[i] * py[j])
                            )
                mi_score = max(0.0, mi)
            except Exception:
                mi_score = 0.0
