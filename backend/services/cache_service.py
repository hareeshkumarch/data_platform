...
class CacheService:
    ...
    async def get(self, key: str) -> Optional[str]:
        val = _STORE.get(key)
        ns = key.split(":", 1)[0]
        (metrics.cache_hits if val is not None else metrics.cache_misses).labels(type=ns).inc()
        if val is None:
            return None
        return val if isinstance(val, str) else json.dumps(val, default=str)

    async def set(self, key: str, value: Any, ttl: int | None = None) -> bool:
        _STORE.set(key, value if not isinstance(value, str) else value, ttl or settings.CACHE_TTL_DEFAULT)
        return True
    ...
    async def set_schema(self, dataset_id: str, schema: Any) -> bool:
        return await self.set_json(f"schema:{dataset_id}", schema, ttl=settings.CACHE_TTL_LLM)
    ...
    async def set_sample(self, dataset_id: str, rows: Any) -> bool:
        return await self.set_json(f"sample:{dataset_id}", rows, ttl=settings.CACHE_TTL_LLM)
    ...
    async def set_eda(self, dataset_id: str, data: Any) -> bool:
        return await self.set_json(f"eda:{dataset_id}", data, ttl=settings.CACHE_TTL_LLM)
    ...
    async def set_insights(self, dataset_id: str, data: Any) -> bool:
        return await self.set_json(f"insights:{dataset_id}", data, ttl=settings.CACHE_TTL_LLM)
    ...
    async def set_charts(self, dataset_id: str, data: Any) -> bool:
        return await self.set_json(f"charts:{dataset_id}", data, ttl=settings.CACHE_TTL_CHART)

    async def cache_chart_config(self, key: str, config: Any) -> bool:
        return await self.set_json(f"chart_cfg:{key}", config, ttl=settings.CACHE_TTL_CHART)
    ...
    async def invalidate_dataset(self, dataset_id: str) -> None:
        for prefix in (f"schema:{dataset_id}", f"sample:{dataset_id}", f"eda:{dataset_id}",
                       f"insights:{dataset_id}", f"charts:{dataset_id}", f"chart_cfg:{dataset_id}"):
