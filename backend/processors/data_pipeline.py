...
class BaseProcessor(ABC):
    ...
    def __call__(self, df: pd.DataFrame, config: Dict[str, Any] = None) -> pd.DataFrame:
        try:
            ...
            logger.info(
                f"{self.name} processor done", rows_in=len(df), rows_out=len(result)
            )
            return result
            ...
            try:
                if pd.api.types.is_numeric_dtype(df[col]):
                    ...
                    df[col] = df[col].fillna(
                        df[col].mode()[0] if len(df[col].mode()) else "unknown"
                    )
            except Exception:
                pass
        return df
...
class DataPipeline:
    ...
    def available_processors() -> List[Dict[str, str]]:
        return [
            {
                "name": "drop_high_null",
                "description": "Drop columns with null% above threshold",
                "config": {"threshold_pct": "int (default 80)"},
            },
            {
                "name": "deduplicate",
                "description": "Remove duplicate rows",
                "config": {"subset": "list of columns (optional)"},
            },
            {
                "name": "type_cast",
                "description": "Cast columns to specified types",
                "config": {"casts": {"col": "datetime|numeric|string|boolean"}},
            },
            {
                "name": "outlier_clip",
                "description": "Clip outliers using IQR or Z-score",
                "config": {"method": "iqr|zscore", "columns": "list (optional)"},
            },
            {
                "name": "impute",
                "description": "Fill nulls with median/mean/mode",
                "config": {"strategy": "median|mean"},
            },
            {
                "name": "normalize",
                "description": "Normalize numeric columns",
                "config": {"method": "minmax|zscore", "columns": "list (optional)"},
            },
            {
                "name": "filter_rows",
            ...
            },
            {
                ...
                "config": {"by": "str|list", "ascending": "bool"},
            },
            {
                "name": "sample",
                "description": "Sample rows",
                "config": {"n": "int", "frac": "float (0-1)", "seed": "int"},
            },
        ]
