...
def _materialize(kind: str) -> str:
    ...
    import asyncio
    ref = asyncio.get_event_loop().run_until_complete(storage.store_file(content, filename))
...
def test_demo_catalogue_lists_all_kinds() -> None:
    ...
    assert {"sales", "ecommerce", "marketing", "finance", "hr", "customers", "iot"}.issubset(kinds)
...
def test_cleaning_suggestions_and_preview() -> None:
    ...
    preview = client.post(f"/api/v1/cleaning/{dataset_id}/preview", json={
